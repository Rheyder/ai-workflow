...
- **Chat** summaries, explanations, **suggested commit messages**, `plan-*` / changelog bullets, and all other prose: **English** (`en_US`) per **`language.mdc`**, except when editing **`rheyder-method-v1.0-pt_BR.md`**. Override only if the human explicitly requests otherwise.
...
6. **Summary to the human** — Branch, changes, commands + results, RED/GREEN if applicable, outcomes of both reviews, slice status in `plan-*` — **English** (`en_US`) per `.cursor/rules/language.mdc` (same for suggested commit text).
