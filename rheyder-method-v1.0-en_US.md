...
| `language` | rule `.mdc` | **pt-BR** in Cursor **chat with the user**; **English** (`en_US`) for all in-repo artifacts and Git **commit messages**. **File:** `.cursor/rules/language.mdc`. |
...
| `language.mdc`                       | persistent context / always-on  | `alwaysApply: true` | Language convention (chat pt-BR; repo + commits `en_US`)                                              | `.cursor/rules/language.mdc` |
